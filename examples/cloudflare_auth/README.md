...
- `bin/server.dart` runs the same app locally with an Ormed-backed SQLite auth
  store at `storage/cloudflare_auth.sqlite` (override with
  `AUTH_DATABASE_PATH`). Local API-key records use the durable companion file
  `<AUTH_DATABASE_PATH>.api-keys` until `server_auth_ormed` adds that capability.
...
To run the Dart IO entrypoint directly with file-backed local data:
...
uses the same durable auth-store contract as the Worker, backed by Ormed
SQLite, while the provider request, profile parsing, and session routes are the
same code used by the Worker.
...
test composition must provide an explicit rate-limit service. The D1 auth store
is not used as an implicit rate-limit backend.
