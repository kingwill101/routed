// Multipart assertions intentionally inspect dynamic decoded form structures.
...
void main() {
  group('parseUrlEncoded comprehensive tests', () {
    ...
    test('should handle arrays with bracket notation', () {
      const input = 'items[]=item1&items[]=item2&items[]=item3';
      final result = parseUrlEncoded(input);
    ...
    test('should decode special characters', () {
      const input = 'special=%40%23%24%25%5E%26*()';
      final result = parseUrlEncoded(input);

      expect(result['special'], equals(r'@#$%^&*()'));
    });

    test('should handle empty values', () {
      const input = 'emptyKey=&anotherEmpty=';
      final result = parseUrlEncoded(input);
    ...
    test('should handle repeated keys with different structures', () {
      const input = 'key=value1&key[]=value2&key[nested]=value3';
      final result = parseUrlEncoded(input);
