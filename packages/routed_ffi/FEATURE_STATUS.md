...
- WebSocket upgrade forwarding over native bridge is not implemented.
- Native asset build/test ergonomics need cleanup so `.dart_tool` native library refresh is always automatic.

## Suggested Next Milestones

1. Implement WebSocket bridge upgrade path.
2. Improve native asset rebuild ergonomics for local test loops and CI caching.
