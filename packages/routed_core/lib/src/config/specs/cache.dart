...
import 'package:routed_core/src/config/spec.dart';
import 'package:routed_core/src/provider/config_utils.dart';
import 'package:routed_core/src/provider/provider.dart';

typedef CacheDriverDocumentationBuilder =
    List<ConfigDocEntry> Function({required String pathBase});
...
class CacheConfigSpec extends ConfigSpec<CacheConfig> {
  const CacheConfigSpec({this.driverDocumentationBuilder});

  final CacheDriverDocumentationBuilder? driverDocumentationBuilder;
  ...
  List<ConfigDocEntry> docs({String? pathBase, ConfigSpecContext? context}) {
    ...
    final builder = driverDocumentationBuilder;
    if (builder != null) {
      docs.addAll(builder(pathBase: path('stores.*')));
    }
    return docs;
  }
