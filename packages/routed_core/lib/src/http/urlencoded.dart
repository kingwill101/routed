...
/// Splits a bracketed key like "key[nested]" into segments
/// ["key","nested"].
/// Also handles "key[]" => ["key",""] for array pushes.
List<String> _splitKeyIntoSegments(String rawKey) {
  // e.g. "user[address][city]" => bracketSplit =
  // ["user","address]","city]"]
...
void _doStore(dynamic current, List<String> segments, String value) {
  ...
  // Not the last segment => we need a container (Map or List) for the next
  // step
  if (head.isEmpty) {
...
/// If a map entry's value is a list of length == 1, replace it with that
/// element.
/// If that element is also a map, recurse. If the value is a map, recurse
/// into it.
