...
/// Minimal configuration contract required by [Request].
abstract interface class RequestConfig {
  bool get forwardedByClientIP;
  bool get enableProxySupport;
  bool get enableSecureRequestIds;
  String? get trustedPlatform;
  List<String> get remoteIPHeaders;
...
class Request implements SessionRequest, RateLimitRequest {
  ...
  Request(this.httpRequest, this.pathParameters, this.config)
    : queryParameters = _safeQueryParameters(httpRequest.uri),
      _attributes = {},
      id = config.enableSecureRequestIds
  ...
  String get clientIP {
    ...
    if (!config.forwardedByClientIP || !config.enableProxySupport) {
