import 'package:routed_openapi/src/openapi/route_manifest.dart';
...
List<String> _mergeTags(List<String>? schemaTags, List<String> extractedTags) {
  final merged = <String>[];
  if (schemaTags != null && schemaTags.isNotEmpty) {
    merged.addAll(schemaTags);
  }
...
List<ParamSchema> _mergeParams(
  ...
  final merged = <ParamSchema>[];
  if (schemaParams != null) {
    merged.addAll(schemaParams);
  }

  for (final param in extractedParams) {
    ...
    merged[existingIndex] = ParamSchema(
      existing.name,
      ...
      description: existing.description.isNotEmpty
          ...
          : param.description,
      jsonSchema: existing.jsonSchema != null && existing.jsonSchema!.isNotEmpty
          ? existing.jsonSchema
          : param.jsonSchema,
      example: existing.example ?? param.example,
    );
  }
...
List<ResponseSchema> _mergeResponses(
  ...
  final merged = <ResponseSchema>[];
  if (schemaResponses != null) {
    merged.addAll(schemaResponses);
  }

  for (final response in extractedResponses) {
    final existingIndex = merged.indexWhere(
    ...
    final existing = merged[existingIndex];
    merged[existingIndex] = ResponseSchema(
      existing.statusCode,
      description: existing.description.isNotEmpty
          ? existing.description
          : response.description,
      contentType: existing.contentType ?? response.contentType,
      jsonSchema: existing.jsonSchema ?? response.jsonSchema,
      headers: existing.headers != null && existing.headers!.isNotEmpty
          ? existing.headers
          : response.headers,
    );
  }
