...
bool shouldLoadProjectCommands(
  ...
  if (selection == 'cli' && values.contains('build')) {
    return false;
  }
  return true;
}

/// Metadata describing a single option provided by a discovered command.
...
/// Metadata describing a discovered project or provider command.
...
/// Proxies discovered commands so they can be exposed through the routed CLI
/// command runner.
...
/// Handles discovery and execution of project and provider-defined commands.
class ProjectCommandsLoader {
  ...
  Future<List<ProjectCommandInfo>> loadProjectCommands(String usage) async {
    ...
    final appEntry = fileSystem.file(
      p.join(projectRoot.path, 'lib', 'app.dart'),
    );
    final hasProjectCommands = commandsEntry.existsSync();
    final hasEngineEntrypoint =
        appEntry.existsSync() && _hasCreateEngineEntrypoint(appEntry);
    if (!hasProjectCommands && !hasEngineEntrypoint) {
      _cached = const [];
      return _cached!;
    }
    ...
    await scriptFile.writeAsString(
      _buildRunnerScript(
        packageName,
        includeEngine: hasEngineEntrypoint,
        includeProjectCommands: hasProjectCommands,
      ),
    );
  ...
  bool _hasCreateEngineEntrypoint(fs.File appFile) {
  ...
  String _buildRunnerScript(
    String packageName, {
    required bool includeEngine,
    required bool includeProjectCommands,
  }) {
    final imports =
        '''
${includeProjectCommands ? "import 'package:$packageName/commands.dart' as project_commands;\n" : ''}${includeEngine ? '''
import 'package:routed_core/routed_core.dart';
import 'package:$packageName/app.dart' as app;
''' : ''}''';
    final mainBody = includeEngine
        ? '''
  final engine = await app.createEngine();
  try {
    final commands = await _loadCommands();
    _loadEngineCommands(commands, engine);
...
'''
  ...
  exit(64);
''';
    final engineCommandLoader = includeEngine
        ? '''
void _loadEngineCommands(List<Command<void>> commands, Engine engine) {
  final registry = engine.container.get<CliCommandRegistry>();
  final existingNames = commands
      .expand((command) => [command.name, ...command.aliases])
      .toSet();
  for (final registration in registry.registrations) {
    final value = registration.factory(engine.container);
    if (value is! Command<void>) {
      stderr.writeln(
        'CLI command registration "\${registration.id}" returned '
        '\${value.runtimeType}, expected Command<void>.',
      );
      exit(64);
    }
    final hasConflict =
...
'''
        : '';
    final projectCommandLoader = includeProjectCommands
        ? '''
