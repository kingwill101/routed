...
Future<bool> _curlSupportsHttp2() async {
  try {
    final result = await Process.run('curl', const ['--version']);
    if (result.exitCode != 0) {
      return false;
    ...
    final output = '${result.stdout}\n${result.stderr}'.toLowerCase();
    return output.contains('http2');
  } on ProcessException {
    return false;
  }
}

Future<ProcessResult> _curlHttp2PriorKnowledgeGet(Uri uri) {
...
void main() {
  ...
  test(
    'serveNative native callback mode forwards websocket upgrade and messages',
    () async {
      final engine = Engine()..ws('/ws', _EchoWebSocketHandler());
  ...
  test(
    'serveNative accepts HTTP/2 prior-knowledge requests when http2 is enabled',
    () async {
      if (!await _curlSupportsHttp2()) {
        markTestSkipped('curl with HTTP/2 support is unavailable');
        return;
      }
      final engine = Engine()..get('/h2', (ctx) async => ctx.string('h2-ok'));
      final running = await _startServer(engine, http2: true);
      final uri = running.baseUri.replace(path: '/h2');

      try {
        final result = await _curlHttp2PriorKnowledgeGet(uri);
        expect(
          result.exitCode,
          0,
          reason:
              'curl failed: stdout=${result.stdout} stderr=${result.stderr}',
        );
        ...
        expect(httpVersion, startsWith('2'));
        expect(body, 'h2-ok');
      } finally {
        await _stopServer(running, engine);
      }
  ...
  test(
    'serveNative rejects HTTP/2 prior-knowledge requests when http2 is disabled',
    () async {
      if (!await _curlSupportsHttp2()) {
        markTestSkipped('curl with HTTP/2 support is unavailable');
        return;
      }
      final engine = Engine()..get('/h2-off', (ctx) async => ctx.string('ok'));
      final running = await _startServer(engine, http2: false);
      final uri = running.baseUri.replace(path: '/h2-off');
      ...
      try {
        ...
        expect(
          ...
          isNonZero,
          reason:
              'expected HTTP/2 prior-knowledge request to fail when http2=false; '
