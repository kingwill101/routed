...
abstract class SimpleCounterCommand implements Command<SimpleCounterModel, SimpleCounter> {
  @override FutureOr<void> postcondition(SimpleCounterModel model, SimpleCounter sut) {
    expect(sut.value, equals(model.value));
  }
}
class SimpleIncrement extends SimpleCounterCommand {
  @override bool precondition(SimpleCounterModel model) => true;
  @override FutureOr<void> run(SimpleCounter sut) async => sut.increment();
  @override SimpleCounterModel update(SimpleCounterModel model) {
      final newModel = model.copy();
      newModel.value++; // Correctly increment the value
      return newModel;
    }
  @override String toString() => 'INC';
}
class SimpleDecrement extends SimpleCounterCommand {
  @override bool precondition(SimpleCounterModel model) => model.value > 0;
  @override void run(SimpleCounter sut) => sut.decrement(); // Explicit void
  @override SimpleCounterModel update(SimpleCounterModel model) {
       final newModel = model.copy();
       if (newModel.value > 0) {
...
abstract class AsyncDataStoreCommand implements Command<AsyncDataStoreModel, AsyncDataStore> {
   // Postcondition: Wait until loading is false before checking state
   @override FutureOr<void> postcondition(AsyncDataStoreModel model, AsyncDataStore sut) async {
      while(sut.loading) {
...
class SetDataCmd extends AsyncDataStoreCommand {
   ...
   @override bool precondition(AsyncDataStoreModel model) => true;
   @override FutureOr<void> run(AsyncDataStore sut) => sut.setData(dataToSet);
   @override AsyncDataStoreModel update(AsyncDataStoreModel model) => model.copy()..data = dataToSet;
   @override String toString() => 'SET($dataToSet)';
}
class ClearDataCmd extends AsyncDataStoreCommand {
   @override bool precondition(AsyncDataStoreModel model) => model.data != null; // Can only clear if data exists
   @override FutureOr<void> run(AsyncDataStore sut) => sut.clearData();
   @override AsyncDataStoreModel update(AsyncDataStoreModel model) => model.copy()..data = null;
...
class BadIncrement extends SimpleCounterCommand {
   @override bool precondition(SimpleCounterModel model) => true;
   @override FutureOr<void> run(SimpleCounter sut) => sut.value += 2; // Incorrect!
   @override SimpleCounterModel update(SimpleCounterModel model) {
        final newModel = model.copy();
        newModel.value++; // Model increments correctly by 1
        return newModel;
     }
   @override String toString() => 'BAD_INC';
}
...
void main() {
  group('Stateful Testing Components', () {
    ...
    test('StatefulPropertyBuilder runs command sequences', () async {
      ...
      final builder = StatefulPropertyBuilder.create<SimpleCounterModel, SimpleCounter>(
        initialModel: () => SimpleCounterModel(),
        setupSut: () async => SimpleCounter(),
        teardownSut: (sut) async { /* No cleanup needed */ },
      )
      .withCommands(Gen.constant(SimpleIncrement()))
      .withCommands(Gen.constant(SimpleDecrement())) // Decrement will only run if valid
      .withConfig(StatefulPropertyConfig(numTests: 50, maxCommandSequenceLength: 10));
     ...
     test('StatefulPropertyBuilder fails on postcondition violation', () async {
       ...
       expect(result.success, isFalse);
              ...
              expect(result.failingInput, isA<CommandSequence>());
              // Check the type of the command that caused the failure
              expect((result.failingInput as CommandSequence).commands.first, isA<BadIncrement>());
            });


     test('StatefulPropertyRunner checks invariants', () async {
        final runner = StatefulPropertyRunner<SimpleCounterModel, Command<SimpleCounterModel, SimpleCounter>>(
          Gen.oneOfGen([Gen.constant(SimpleIncrement()), Gen.constant(SimpleDecrement())]),
          () => SimpleCounterModel(),
     ...
     test('StatefulPropertyRunner fails on invariant violation', () async {
        final runner = StatefulPropertyRunner<SimpleCounterModel, Command<SimpleCounterModel, SimpleCounter>>(
          ...
          // Apply update regardless of precondition, forcing potential negative value
                    ...
                    StatefulPropertyConfig(numTests: 10, maxCommandSequenceLength: 5) // Fail fast
                  );

        final result = await runner.run();
        expect(result.success, isFalse);
        expect(result.error, isA<Exception>());
        expect((result.error as Exception).toString(), contains('led to state violating invariant'));
        expect((result.failingInput as List).first, isA<SimpleDecrement>());
     });

     test('StatefulPropertyBuilder handles async commands and postconditions', () async {
       // Corrected: Call the static factory 'create'
       final builder = StatefulPropertyBuilder.create<AsyncDataStoreModel, AsyncDataStore>(
         initialModel: () => AsyncDataStoreModel(),
         setupSut: () async => AsyncDataStore(),
         teardownSut: (sut) async {},
       )
       .withCommands(Gen.string(minLength: 1, maxLength: 10).map((s) => SetDataCmd(s)))
       .withCommands(Gen.constant(ClearDataCmd()))
       .withConfig(StatefulPropertyConfig(numTests: 30, maxCommandSequenceLength: 8));
    ...
    // Basic Shrinking Test for Stateful (more involved tests are harder)
     test('Stateful shrinking removes commands', () async {
        ...
        final commands = [SimpleIncrement(), SimpleDecrement(), SimpleIncrement()];
        final sequence = CommandSequence(commands);

        // Use base PropertyTestRunner for this specific shrinking test scenario
        final runner = PropertyTestRunner<CommandSequence<SimpleCounterModel, SimpleCounter>>(
           ...
           (seq) async {
             ...
             if(seq.commands.length >= 2 && seq.commands[0] is SimpleIncrement && seq.commands[1] is SimpleDecrement) {
        ...
        final shrunkCommands = (result.failingInput as CommandSequence).commands;
                ...
                expect(shrunkCommands.length, lessThanOrEqualTo(3), reason: "Shrunk sequence should be shorter than original");
                expect(shrunkCommands.length, greaterThanOrEqualTo(2), reason: "Should contain the minimal failing prefix [INC, DEC]");
