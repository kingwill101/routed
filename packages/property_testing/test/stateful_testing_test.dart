...
class ClearDataCmd extends AsyncDataStoreCommand {
  @override
  bool precondition(AsyncDataStoreModel model) => model.data != null; // Can only clear if data exists
  @override
...
void main() {
  group('Stateful Testing Components', () {
    test('StatefulPropertyBuilder runs command sequences', () async {
      ...
      final builder =
          StatefulPropertyBuilder.create<SimpleCounterModel, SimpleCounter>(
              ...
              ) // Decrement will only run if valid
              .withConfig(
                StatefulPropertyConfig(
                  numTests: 50,
                  maxCommandSequenceLength: 10,
                ),
              );
    ...
    test('StatefulPropertyBuilder fails on postcondition violation', () async {
      ...
      expect(
        (result.failingInput as CommandSequence).commands.first,
        isA<BadIncrement>(),
      );
    });
    ...
    test(
      'StatefulPropertyBuilder handles async commands and postconditions',
      () async {
        // Corrected: Call the static factory 'create'
        final builder =
