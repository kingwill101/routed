...
void main() {
  group('DateTime Generator', () {
    ...
    test('respects UTC flag', () async {
      final runner = PropertyTestRunner(Specialized.dateTime(utc: true), (
        date,
      ) {
    ...
    test('generates non-UTC dates when UTC is false', () async {
      final runner = PropertyTestRunner(Specialized.dateTime(utc: false), (
        date,
      ) {
    ...
    test('shrinks towards epoch while respecting minimum', () async {
      ...
      final runner = PropertyTestRunner(Specialized.dateTime(min: min), (date) {
      ...
      expect(
        (result.failingInput as DateTime).isBefore(min),
        isFalse,
        reason: 'Shrunk value should not be before min',
      );
      expect(
        (result.failingInput as DateTime).isBefore(DateTime(2024)),
        isTrue,
    ...
    test('handles far future dates', () async {
      ...
      final runner = PropertyTestRunner(Specialized.dateTime(max: farFuture), (
        date,
      ) {
        expect(
    ...
    test('preserves millisecond precision', () async {
      ...
      final runner = PropertyTestRunner(Specialized.dateTime(), (date) {
        seenMilliseconds.add(date.millisecond);
      }, PropertyConfig(numTests: 1000));

      await runner.run();
      expect(
        seenMilliseconds.length,
        greaterThan(50),
      ); // Should see many different milliseconds
    });
