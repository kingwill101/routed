...
void main() {
  group('Email Generator', () {
    ...
    test('respects domain constraints', () async {
      ...
      final runner = PropertyTestRunner(
        Specialized.email(domains: domains),
        (email) {
    ...
    test('generates diverse local parts', () async {
      ...
      expect(seenChars.length,
          greaterThan(20)); // Should see many different characters
      expect(seenLengths.length,
          greaterThan(10)); // Should see many different lengths
    });
    ...
    test('handles edge cases', () async {
      ...
      final runner = PropertyTestRunner(
        ...
        (email) {
          final localPart = email.split('@')[0];
          try {
            expect(localPart.length, equals(1));
            expect(
                email,
                matches(
      ...
      final result = await runner.run();
      if (!result.success) {
        print('Test failed with input: ${result.failingInput}');
      ...
      final runner = PropertyTestRunner(
        ...
        (email) {
          ...
          expect(parts[0].length,
              lessThanOrEqualTo(64)); // Maximum local part length
          expect(
              parts[1].length, lessThanOrEqualTo(255)); // Maximum domain length
        },
        PropertyConfig(numTests: 1000),
      );
