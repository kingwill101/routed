...
class MyCustomList<T> extends ListBase<T> {
  ...
  MyCustomList(Iterable<T> items, this.metadata) : _internalList = List.from(items);
  ...
  @override
  String toString() => 'MyCustomList(metadata: "$metadata", items: $_internalList)';
}
  ...
  group('Gen.containerOf', () {
    ...
    test('generates Set<String> respecting constraints', () async {
      ...
      final runner = PropertyTestRunner(gen, (set) {
        ...
        expect(set.length, greaterThanOrEqualTo(0)); // Can be 0 if all generated are same
    ...
    test('shrinks container by removing elements', () async {
       ...
       expect((result.failingInput as Set<int>).length, lessThanOrEqualTo((result.originalFailingInput as Set<int>).length));
       // Check if it shrunk towards the failure boundary (<= 3)
       expect((result.failingInput as Set<int>).length, lessThanOrEqualTo(4)); // Should ideally be <= 3, allow 4 for intermediate shrinks
    });
    ...
    test('shrinks container by shrinking elements', () async {
       ...
       expect((result.failingInput as List<int>).length, equals(1)); // Length shouldn't change
       ...
       expect(failingElement, greaterThan(15), reason: "Shrunk value should still fail the check");
    });

    test('throws ArgumentError for invalid min/max length', () {
      expect(
        () => Gen.containerOf<List<int>, int>(Gen.integer(), (i) => [], minLength: 5, maxLength: 4),
        throwsArgumentError,
      );
       expect(
        () => Gen.containerOf<List<int>, int>(Gen.integer(), (i) => [], minLength: -1),
        throwsArgumentError,
      );
       expect(
        () => Gen.containerOf<List<int>, int>(Gen.integer(), (i) => [], maxLength: -1),
        throwsArgumentError,
      );
    });
