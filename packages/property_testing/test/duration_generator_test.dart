...
void main() {
  group('Duration Generator', () {
    ...
    test('generates non-negative durations by default', () async {
      final runner = PropertyTestRunner(Specialized.duration(), (duration) {
    ...
    test('respects minimum duration', () async {
      ...
      final runner = PropertyTestRunner(Specialized.duration(min: min), (
        duration,
      ) {
    ...
    test('shrinks towards zero while respecting minimum', () async {
      ...
      expect(
        result.failingInput.inMicroseconds,
    ...
    test('handles microsecond precision', () async {
      ...
      await runner.run();
      expect(
        seenMicroseconds.length,
        greaterThan(50),
      ); // Should see many different microseconds
    });
    ...
    test('handles large durations', () async {
      ...
      final runner = PropertyTestRunner(Specialized.duration(max: max), (
        duration,
      ) {
        expect(duration.inMicroseconds, lessThanOrEqualTo(max.inMicroseconds));
        expect(
          duration * 2,
