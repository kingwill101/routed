  ...
  group('Gen.frequency', () {
    ...
    test('works with different generator types', () async {
     ...
     test('shrinking delegates to the chosen generator', () async {
        final failingGen = Gen.integer(min: 10, max: 20).where((x) => x > 15); // Fails if > 15
        ...
        }, PropertyConfig(numTests: 50)); // Run enough to likely hit the failing gen

        final result = await runner.run();

        expect(result.success, isFalse);
        expect(result.originalFailingInput, greaterThan(15));
        expect(result.failingInput, greaterThan(15), reason: "Shrunk value must still fail");
      ...
      test('handles complex nested frequency generators', () async {
        ...
        final nestedFreq = Gen.frequency([(1, leafGen), (1, Gen.constant('nested'))]);
        final topFreq = Gen.frequency([(1, nestedFreq), (1, Gen.constant('top'))]);
