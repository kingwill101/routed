...
/// final client = EngineTestClient(yourRequestHandler);
///
/// // Send a GET request
...
class TestClient {
  ...
  /// final client = EngineTestClient(myHandler);
  ///
  /// // Create a client with a real HTTP server
  /// final serverClient = EngineTestClient(
  ///   myHandler,
  ///   mode: TransportMode.ephemeralServer,
  /// );
  /// ```
  factory TestClient(RequestHandler handler,
  ...
  /// [handler] is the request handler that will process the HTTP requests.
  /// [options] provides additional configuration options for the transport.
  TestClient.ephemeralServer(RequestHandler handler,
      [TransportOptions? options])
  ...
  ///
  /// // Then in your test:
  /// final client = EngineTestClient(UserHandler());
  /// final response = await client.get('/users');
  ...
  ///
  /// Returns a [TestResponse] that can be used for assertions.
  Future<TestResponse> request(String method, String uri,
          [Map<String, List<String>>? headers]) =>
