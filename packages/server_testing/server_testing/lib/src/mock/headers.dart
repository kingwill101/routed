...
MockHttpHeaders setupHeaders(Map<String, List<String>> requestHeaders) {
  ...
  final contentTypeHeaderName = normalizeHeaderName(
  ...
  // Handle add: append values (HttpHeaders keeps list of values for a header)
  ...
  when(mockRequestHeaders.removeAll(any)).thenAnswer((invocation) {
    ...
    requestHeaders.removeWhere(
      (k, _) => normalizeHeaderName(k) == normalizedName,
    );
  });

  // Handle set: replace values for that header
