...
class TestResponse {
  ...
  void _parseJsonIfApplicable() {
    ...
    if (ct[0].contains('application/json') as bool) {
      try {
        ...
        throw TestFailure(
          'Failed to parse JSON body: $body\nError: $err\nContent-Type: $ct',
        );
      }
    }
  }

  /// Returns a parsed Cookie by name from Set-Cookie headers, if present.
  Cookie? cookie(String name) {
    final setCookies = headers[HttpHeaders.setCookieHeader] ?? const <String>[];
    for (final v in setCookies) {
      try {
        final c = Cookie.fromSetCookieValue(v);
  ...
  TestResponse assertHeaderContains(String key, dynamic value) {
    ...
    if (value is String) {
      if (actual.isEmpty || !actual.any((h) => h.contains(value))) {
        throw TestFailure(
      ...
      for (var v in value) {
        if (actual.isEmpty || !actual.any((h) => h.contains(v as String))) {
          throw TestFailure(
  ...
  /// Asserts that the Content-Type header matches the specified [contentType].
  ///
  /// Throws a [TestFailure] if the Content-Type header does not match the [contentType].
  TestResponse assertContentType(String contentType) {
    return assertHeader(HttpHeaders.contentTypeHeader, contentType);
  }

  /// Asserts that the body is empty.
  ///
  /// Throws a [TestFailure] if the body is not empty.
  TestResponse assertBodyIsEmpty() {
    if (bodyBytes.isNotEmpty) {
      throw TestFailure('Expected body to be empty, but got "$body".');
    }
    return this;
  }

  /// Asserts that the status code matches the specified [statusCode].
  ///
  /// Throws a [TestFailure] if the status code does not match.
  TestResponse assertStatusCode(int statusCode) {
    if (statusCode != this.statusCode) {
      throw TestFailure(
        'Expected status code to be $statusCode, but got ${this.statusCode}.',
      );
    }
    return this;
  }
  ...
  ///
  /// Throws a [TestFailure] if the status code is not in the successful range.
  TestResponse assertSuccess() {
    if (statusCode < 200 || statusCode > 299) {
      throw TestFailure(
        'Expected status code to be successful (200-299), but got $statusCode.',
      );
    }
    return this;
  }

  /// Asserts that the response is a client error (status code 400-499).
  ///
  /// Throws a [TestFailure] if the status code is not in the client error range.
  TestResponse assertClientError() {
    if (statusCode < 400 || statusCode > 499) {
      throw TestFailure(
        'Expected status code to be a client error (400-499), but got $statusCode.',
      );
    }
    return this;
  }

  /// Asserts that the response is a server error (status code 500-599).
  ///
  /// Throws a [TestFailure] if the status code is not in the server error range.
  TestResponse assertServerError() {
    if (statusCode < 500 || statusCode > 599) {
      throw TestFailure(
        'Expected status code to be a server error (500-599), but got $statusCode.',
      );
    }
    return this;
  }

  /// Asserts that the response has no body.
  ///
  /// Throws a [TestFailure] if the body exists.
  TestResponse assertNoBody() {
    if (bodyBytes.isNotEmpty) {
      throw TestFailure('Expected no body, but got "$body".');
    }
    return this;
  }
  ...
  ///
  /// Throws a [TestFailure] if the headers are empty.
  TestResponse assertHasHeaders() {
    if (headers.isEmpty) {
      throw TestFailure('Expected response to have headers, but found none.');
    }
    return this;
  }

  /// Asserts that the response has no headers.
  ///
  /// Throws a [TestFailure] if any headers are present.
  TestResponse assertNoHeaders() {
    if (headers.isNotEmpty) {
      throw TestFailure(
        'Expected response to have no headers, but found ${headers.length}.',
      );
    }
    return this;
  }
  ...
  ///
  /// Throws a [TestFailure] if the body does not match the [expectedBody].
  TestResponse assertBody(String expectedBody) {
    return assertBodyEquals(expectedBody);
  }
  ...
  TestResponse assertJsonPath(String path, dynamic expected) {
    ...
    if (expected is Matcher) {
      expect(
        ...
        expected,
        reason:
            'Expected JSON path "$path" to match matcher "$expected", but got "$actual".',
      );
