...
2. Test structure and values:
```dart
json
  .matchesSchema({'user': Map})
  .scope('user', (user) {
    user.where('name', 'John');
  });
```

3. Use scoping for clarity:
```dart
// Good
json.scope('user', (user) {
  user.where('name', 'John');
...
// Less Good
json.where('user.name', 'John');
```

4. Chain assertions fluently:
```dart
json
  .has('id')
  .whereType<int>('id')
