...
// Less Good - Less clear intent
number.assertGreaterThan(0);
number.assertMultipleOf(5);
```

2. Custom Messages for Clarity:
```dart
// Good - Clear context
age.assertGreaterThan(18, 
  message: 'User must be an adult (18+)'
);

// Less Good - Default message only
age.assertGreaterThan(18);
```
...
// Less Good - Mixed concerns
number.assertGreaterThan(0);
number.assertEven();
number.assertLessThan(100);
```

4. Use Boolean Methods for Logic:
```dart
// Good - Use boolean methods in conditions
if (number.isGreaterThan(threshold)) {
  // Additional validation
}

// Less Good - Direct comparison
if (number > threshold) {
