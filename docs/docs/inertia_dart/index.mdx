...
- `dart:io` helpers for routing, HTML rendering, and static asset serving
...
Use this flow when you want a plain `HttpServer` app with no framework layer.
...
The server reads `client/public/hot` during development and switches to
`client/dist/.vite/manifest.json` after a production build, so the same Dart
code works in both modes.

To add SSR later, start a renderer with `dart run bin/ssr.dart` or
`dart run inertia_dart:inertia ssr:start`, then pass an `ssr:` callback into
`respondWithInertiaPage()` and forward the returned payload into
`renderInertiaVitePageHtml(..., ssr: ssr)`.

For a full working server, see the [tutorial](/docs/inertia_dart/tutorial) or
the example apps in [`packages/inertia/example`](https://github.com/kingwill101/routed/tree/main/packages/inertia/example).
