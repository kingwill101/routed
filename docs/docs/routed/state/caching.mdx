...
Extend caching by registering your own store factories (see [Contracts](/docs/routed/fundamentals/contracts)):

Custom stores should implement the `Store` contract or extend `TaggableStore` if you want tag-aware invalidation. Once registered, access the store exactly like the built-in array and file drivers.
