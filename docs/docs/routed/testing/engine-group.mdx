...
By default, engines are created with `Engine.defaultProviders` (core + routing).
Pass `providers` to use a different set:
...
engineGroup(
  ...
  providers: [
    ...
    RoutedSessionsProvider(SessionConfig.cookie(appKey: 'secret')),
  ],
  options: [
    withCacheManager(
      DataCacheManager()
    ...
    withMiddleware([sessionMiddleware()]),
...
engineGroup(
  'bare engine tests',
  providers: [],  // No providers, bare engine
  define: (engine, client, test) {
    // add routes and tests here
  },
);
...
If you need full control, pass typed providers and wiring options to shape every
engine created by the group. (Each test still receives a fresh engine
instance.)
...
Pass `transportMode: TransportMode.ephemeralServer` to run each test against a real `HttpServer` instead of the default in-memory transport—useful when middleware or proxies rely on actual sockets.
