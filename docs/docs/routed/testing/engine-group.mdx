...
engineGroup(
  'with cache & sessions',
  providers: Engine.builtins,  // Use all providers
...
If you need full control, pass `engineConfig` and `options` to shape every engine created by the group. (Each test still receives a fresh engine instance.)
