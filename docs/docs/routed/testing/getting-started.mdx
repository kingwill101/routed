...
`engineTest` spins up a fresh engine and client for each test. You can pass `engine`, `providers`, `configItems`, `engineConfig`, `options`, or `autoCloseEngine` when you need to customize the engine lifecycle. By default, engines are created with `Engine.defaultProviders` (core + routing). Use `transportMode: TransportMode.ephemeralServer` when you need to exercise the full HTTP stack instead of the default in-memory transport.
