...
`engineTest` spins up a fresh engine and client for each test. You can pass
`engine`, typed `providers`, `engineConfig`, `options`, or `autoCloseEngine`
when you need to customize the engine lifecycle. By default, engines are
created with the typed core and routing providers. Use
`transportMode: TransportMode.ephemeralServer` when you need to exercise the
full HTTP stack instead of the default in-memory transport.
...
See the [Testing index](/docs/routed/testing/overview) for additional helpers.
