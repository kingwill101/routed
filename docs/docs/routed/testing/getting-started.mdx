...
`engineTest` spins up a fresh engine and client for each test. Use `transportMode: TransportMode.ephemeralServer` when you need to exercise the full HTTP stack instead of the default in-memory transport.
...
Because `engineTest` uses `server_testing` under the hood, you can share the same handler with browser automation:

```dart
engineTest('renders home page', (engine, client) async {
