...
Key APIs include the container, scoped bindings, and provider lifecycle:
...
Attach providers to the engine via `registerProvider`. Providers run in three phases:
...
The built-in `CoreServiceProvider` wires `EngineConfig` + `Config` via the config loader, while `RoutingServiceProvider` configures routing events. Caching and view engines live in the `routed.cache` and `routed.views` providers. These built-ins are available via `Engine.defaultProviders`. Providers can also opt into `ProvidesDefaultConfig` to advertise defaults to the global `ConfigRegistry`, allowing the loader to merge them automatically. If you want to control which providers boot, pass them when constructing the engine:
...
`RequestServiceProvider` registers the HTTP request/response, the `Request`/`Response` wrappers, and `EngineContext`. Any binding marked with `container.scoped()` will be disposed when the request completes.

### Fast-path containers (performance mode)

If you enable `EngineFeatures.enableRequestContainerFastPath`, Routed skips
creating per-request child containers. Instead it exposes a **read-only**
view of the root container and stores `Request`, `Response`, and
`EngineContext` in an internal request scope.

Implications:
- `container.bind`, `container.instance`, `container.scoped`, and similar
  mutations will throw a `StateError`.
- Request-scoped state should live on `EngineContext` (or your own middleware
  state), not in the container.
- This mode is intended for benchmarks or ultra-low-latency services where
  dependency injection per request is unnecessary.

## AppZone helpers

Code that runs outside a request can still reuse configuration, routing, and logging helpers through `AppZone`.
...
- `configValue<T>(key)` reads from the app configuration.
