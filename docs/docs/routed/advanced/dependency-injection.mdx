...
The built-in `CoreServiceProvider` wires the typed `EngineConfig`, while
`RoutingServiceProvider` configures routing events. Caching and view engines
live in their adapter packages. Providers can expose a typed configuration with
`ProvidesTypedConfiguration<T>`. If you want to control which providers boot,
pass them when constructing the engine:
