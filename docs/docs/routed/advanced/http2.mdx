...
HTTP/2 requires TLS. Provide your certificate and pass a typed `Http2Config`:
...
- Request logging and sessions work without modification; the opt-in gzip
  provider compresses eligible buffered responses. Brotli and transport-level
  compression remain host or reverse-proxy concerns.
- WebSockets remain an HTTP/1.1-only feature; clients attempting to upgrade over HTTP/2 will receive the documented error.

## Graceful shutdown

The existing shutdown controller drains active HTTP/2 streams before closing the listener. Fallback to HTTP/1.1 is handled automatically—no additional steps are required in deployment scripts.
...
- Add your CA bundle to the client `SecurityContext` when writing integration tests.
