...
`routed_storage` currently creates local disks from `storage.*` configuration;
it does not expose a generic storage driver registry. For a custom or cloud
disk, construct a `StorageDisk` and pass a configured `StorageManager` to
...
Use `RoutedStaticProvider` alongside it when the app also needs
`static.mounts`.
...
CacheManager.registerDriver(
  ...
  configBuilder: (context) {
    ...
    config['cache_dir'] ??= 'storage/framework/cache/redis';
    return config;
  },
  ...
  documentation: (ctx) => <ConfigDocEntry>[
    ...
    ConfigDocEntry(
      ...
      metadata: const {
        'default_note': 'Defaults to storage/framework/cache/redis.',
      },
    ),
  ],
);
...
- **Config builders:** `DriverConfigContext` exposes the raw configuration, driver name, and a `get<T>()` helper for container lookups.
...
SessionServiceProvider.registerDriver(
  ...
  (context) {
    ...
    final resolvedRoot = (root == null || root.trim().isEmpty)
        ? 'storage/framework/sessions/dropbox'
        : root;
...
- **Context:** `SessionDriverBuilderContext` exposes the merged config and container access so builders can derive paths or look up other services.
...
import 'package:routed/routed.dart'; // exposes LogDriverRegistry
