...
The framework registers an `EventManager` singleton via `RoutingServiceProvider`. Resolve it from the container or the request context when publishing or subscribing:
...
`EventManager` wraps `package:event_bus/event_bus.dart` and implements `Disposable` so it cleans up on engine shutdown. Listener lifecycle: keep references to `StreamSubscription`s and cancel them during provider `cleanup()` or test teardown to avoid leaks. For one-shot reactions prefer `.first`/`take(1)` or filter-and-`takeWhile(...)`. Always supply `onError` when listening so failures don't go unobserved.
...
Routed emits structured events during request processing whenever the routing provider is active:
...
> Prefer the signal types re-exported by `package:routed/routed.dart`; the public Routed package intentionally exposes one central barrel.
...
The engine also publishes system-level events:
...
## Typed configuration lifecycle

Typed provider configuration is assembled before `Engine.initialize()` and is
immutable for the lifetime of the engine. If deployment settings change,
construct a new engine with a new `RuntimeContext` and provider list rather
than relying on a configuration reload event.
