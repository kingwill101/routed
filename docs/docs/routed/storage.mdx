...
bare endpoint host defaults to HTTPS; an explicit URL can select HTTP and a
custom port for local development. The constructor does not contact the
service or create the bucket.
...
without casting the selected disk. Use `ctx.cloudStorage()` or
`manager.cloud()` for cloud-specific and temporary URL operations. The manager keeps
this API scoped to the Routed application instead of initializing
`storage_fs`'s process-global `Storage` facade.
