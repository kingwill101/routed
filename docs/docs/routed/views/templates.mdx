...
`ViewEngine` is an interface, so you can plug in any renderer you like. Swap the default engine with your own implementation:
...
For projects that need more than one templating language, grab the shared `ViewEngineManager` and register additional engines by extension:
