...
```bash
dart pub add routed
dart pub add --dev routed_cli routed_testing server_testing server_testing_shelf
```

> `routed_cli` ships the scaffolding, dev server, manifest inspection, and provider utilities. Keep the framework package (`routed`) in `dependencies`; it is imported by the application. Run the CLI with `dart run routed_cli ...`.
...
The generator writes the standard `bin/server.dart`, configuration files under `config/`, and project metadata. It also runs `dart pub get` automatically—if dependency resolution fails, the CLI prints the exit code along with the command to rerun manually. Choose a template with `--template` (`basic`, `api`, `web`, or `fullstack`), supply `--output <directory>` to scaffold into an existing folder, and pass `--force` to overwrite conflicting files. Every template includes `lib/commands.dart`; populate its `buildProjectCommands()` hook and the Routed CLI will surface your custom commands in `dart run routed_cli --help`.

> Tip: Need to regenerate config stubs later? Use `dart run routed_cli config:publish` to recreate the defaults, or `dart run routed_cli config:publish app,cache` to publish a smaller subset.
...
Future<void> main() async {
  ...
  // Create a fully-featured engine with all official providers
...
```dart
registerRoutedProviders();
// Full-featured engine with all official providers (recommended for most apps)
final engine = await Engine.create();
...
> Tip: Routed ships feature toggles in each provider config file and a CLI (`dart run routed_cli provider:list --config`) that shows provider defaults and configuration validation errors. Use it any time you adjust caching, sessions, or security settings.
...
```bash
dart run routed_cli openapi generate --output .dart_tool/routed/route_manifest.json
```
...
Routed includes a Liquid template engine. After calling
`registerRoutedProviders()`, `Engine.create()` loads the view provider. Just
