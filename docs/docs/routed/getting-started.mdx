...
The generator writes the standard `bin/server.dart`, configuration files under `config/`, and project metadata. It also runs `dart pub get` automatically—if dependency resolution fails, the CLI prints the exit code along with the command to rerun manually. Supplying `--output <directory>` lets you scaffold into an existing folder, and `--force` overwrites conflicting files.
