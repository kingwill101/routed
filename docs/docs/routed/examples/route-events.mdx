...
> The event manager is registered by `RoutingServiceProvider`. Include that
> typed provider when composing a slim engine; `Engine.create()` with the
> normal defaults includes it already.
