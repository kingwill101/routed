...
- Without `bail`, Routed now accumulates every failing rule message for that field—responses contain an ordered list instead of just the last failure.
...
> Note: `ValidationRuleRegistry` is registered by `RoutingServiceProvider`. If you construct `Engine(includeDefaultProviders: false)`, register that provider (or register the registry manually) before calling `engine.container.get<ValidationRuleRegistry>()`.
