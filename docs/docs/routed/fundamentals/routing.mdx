...
The same helpers are available on the engine. When using `staticFile`/`static`,
paths are resolved relative to the current working directory—never interpolate
untrusted input into these paths.

For declarative mounts, use the typed storage and static providers:
  ...
  providers: [
    ...
    RoutedStorageProvider(
      configuration: StorageConfig(
        defaultDisk: 'assets',
        disks: {
          'assets': const LocalStorageDiskConfig(root: 'public/assets'),
        },
      ),
    ),
...
Each mount is validated to stay inside its disk or root. Directory listings
are disabled by default; enable them only for intentionally public content.
...
### Typed routing defaults

Pass routing defaults to `RoutingServiceProvider`:
  ...
  providers: [
    ...
    RoutingServiceProvider(
      const RoutingConfig(
        ...
        etagStrategy: EtagStrategy.weak,
      ),
    ),
  ],
);
...
The provider validates and applies the immutable `RoutingConfig` during boot.
