...
> Note: `RoutePatternRegistry` is available by default in bare mode (`Engine()`) and is also registered by `RoutingServiceProvider` in full mode. If you manually strip registries, re-register one before calling `engine.container.get<RoutePatternRegistry>()`.
...
Groups can be nested to any depth and their paths are prefixed cumulatively. Middleware declared on a group runs after any parent middleware and before route-level middleware. The runtime route manifest and OpenAPI builder flatten the complete hierarchy into the paths the application serves.
...
Import `package:routed_openapi/routed_openapi.dart` and use fluent `RouteBuilder` extensions to attach metadata that flows into the generated manifest and CLI tooling:
...
```bash
dart run routed openapi generate --output openapi.json
```
...
Prefer configuration? Enable the static provider and describe mounts in `config/http.yaml` (older manifests) or `config/static.yaml` (current scaffolds):
...
Run `dart run routed provider:list --config` to confirm the static provider is enabled and to inspect the mounts after merging defaults—misconfigured mounts emit `ProviderConfigException` during boot. When enabled, mounts also appear in `dart run routed routes` as `/{*filepath}` routes.
...
At runtime you can still override behaviour with `EngineConfig` or helper options (`withRedirectTrailingSlash`, `withHandleMethodNotAllowed`). Run `dart run routed provider:list --config` to confirm the routing provider is active and to review the merged defaults.
