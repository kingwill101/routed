...
The same helpers are available on the engine. When using `staticFile`/`static`, paths are resolved relative to the current working directory—never interpolate untrusted input into these paths. For configuration-driven mounts, enable `routed.static` and describe mounts in `config/static.yaml`:
...
`routed` facade, or compose `RoutedStorageProvider` and `RoutedStaticProvider`
directly when using split packages. `provider:list --config` should then show
`routed.static`.
