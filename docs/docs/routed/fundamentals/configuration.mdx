...
When you publish a trimmed config file (for example via `config:publish app,cache`), any omitted keys continue to resolve from provider defaults. Only keys present in `config/` (or runtime overrides) replace the defaults.
...
## YAML files are optional

You do **not** need a `config/` directory, YAML files, or `.env` files to use Routed's
configuration system. Both `Engine()` and `Engine.create()` accept a `configItems`
parameter that is merged directly on top of the framework defaults — no file I/O happens
at all. This is the simplest way to get started and is perfectly valid for production use:

```dart
// Register the official catalogue before using the full Engine.create() setup.
registerRoutedProviders();
final engine = await Engine.create(
  configItems: {
    'app.name': 'My App',
    ...
    'session.config.app_key': 'base64:abc123...',
  },
);
```

The same works with the synchronous constructor:
...
Under the hood, `configItems` creates a `CoreServiceProvider` with those values and
prepends it to the providers list. The engine deduplicates providers by type, so any
`CoreServiceProvider` in `Engine.builtins` is automatically skipped.

### When you need more control

If you need the file-based loader or a cached config snapshot, construct the
`CoreServiceProvider` explicitly:

| Constructor | File I/O | Use case |
| --- | --- | --- |
| `Engine(configItems: {...})` | None | Inline config only — no YAML, no `.env` |
| `CoreServiceProvider.withLoader(options)` | Reads `config/` and `.env` | Full loader with file watching, Liquid templates, etc. |
| `CoreServiceProvider.withCachedConfig(snapshot)` | None | Pre-compiled snapshot from `config:cache` for zero-I/O startup |

All three approaches accept `configItems` as an optional override map that is always
merged last (highest priority). Even when using the loader, you can pass `configItems` to
override specific keys without touching any file on disk.

### Dot-notation keys in `configItems`

`configItems` accepts both nested maps and flat **dot-notation** keys. The framework
automatically expands dotted keys into nested structures, so these two forms are
equivalent:

```dart
// Dot notation — flat and concise
final engine = await Engine.create(
  configItems: {
    'jwt.enabled': true,
    'jwt.secret': 's3cret',
...
// Nested maps — mirrors the YAML structure
final engine = await Engine.create(
  configItems: {
    'jwt': {
      'enabled': true,
...
Dot notation is especially useful when you only need to flip a handful of flags without
constructing deeply nested maps:
...
:::tip
When a dotted key and a nested map target the same path, they are deep-merged — not
overwritten. You can safely combine them in a single `configItems` map.
:::

### Late overrides with `withConfigItems`

The `withConfigItems` engine option lets you override config values *after* providers
have registered their defaults. This is handy when you want to combine it with other
engine options:
...
:::note
The `configItems` constructor parameter and `withConfigItems` option serve different
timing needs. The constructor parameter is available *during* provider registration
(before boot), while `withConfigItems` runs *after* all providers have registered but
*before* boot.
:::

After `registerRoutedProviders()`, `Engine.create` constructs an engine with
all official providers, boots service providers, and returns the initialized
instance. Pass a custom
`providers` list to override. If you need finer control (for example, registering
listeners before boot), you can instantiate `Engine(providers: [...])` and call
`await engine.initialize()` manually.

The loader pushes a resolved `ConfigImpl` instance into the container. A lightweight `ScopedConfig` overlay is created for every
request so per-request mutations do not leak back to the application-level config. Reads fall through to the parent; writes are captured in a local override map.
...
## Provider features & CLI tooling

Every Routed project uses `config/http.yaml` to toggle provider-driven features. The engine reads these flags during boot, and the CLI uses the same config to surface defaults and validation errors.
...
The output includes each provider’s `configSource` and the default configuration it publishes. Add a provider id (for example `dart run routed_cli provider:list routed.logging --config`) to filter to a single provider. If the manifest includes malformed overrides (wrong types, missing maps, etc.), Routed throws a `ProviderConfigException` and the CLI shows the failing path so you can fix it before starting the server.
...
Use `config:publish` to generate the default templates into `config/`:

```bash
dart run routed_cli config:publish
```

To publish only a subset, pass a comma-separated list (or the `--only` flag). The CLI generates only the matching `config/<name>.yaml` files and limits `.env`/`.env.example` entries to the selected roots:
...
This is the provider catalogue for the current `package:routed` bundle. The
The `routed.security` provider contributes request-size limits, CORS,
trusted-proxy settings, and optional IP filtering. The `routed.compression`
provider adds opt-in gzip compression for buffered responses. The
`routed.static` provider serves storage-backed declarative mounts, while direct
engine/router static helpers remain available for code-defined routes.
...
| `routed.localization` | Binds the translation loader + translator using the default filesystem and `translation.*` config. |
| `routed.views` | Resolves view engines/directories using the shared storage layer (`view.*`). |
| `routed.compression` | Adds opt-in gzip compression for eligible buffered responses (`compression.*`). |
| `routed.security` | Applies request-size limits, CORS, trusted-proxy client-IP resolution, and optional IP filtering (`security.*` and `cors.*`). |
...
for (final provider in providers) {
  ...
  for (final field in provider.fields) {
    logger.debug('${field.path}: ${field.description}', {
...
- `uploads.max_memory` / `uploads.max_file_size` must be integers.
  - `cors.allowed_origins`, `logging.request_headers`, and `security.trusted_proxies.proxies` must be non-empty strings. `static.mounts` entries must provide a route and either a disk or root.
...
The currently registered providers honour these flags when deciding whether to
run. `routed.security` applies the proxy, CORS, and IP-filter settings described
above; CSRF and security-header middleware remain explicit application APIs.
...
### `configNamespace` helper

Use `configNamespace()` to retrieve an entire configuration subtree as a map:

```dart
import 'package:routed/routed.dart';

// Get the entire 'mail' config subtree
final mailConfig = configNamespace('mail');
// => {'host': 'smtp.example.com', 'port': 587, ...}

// Get all config
final allConfig = configNamespace(null);
```

This is useful when passing a whole section to a service constructor (e.g. `Mailer.fromConfig(configNamespace('mail'))`).

### Automatic `app.root` injection

The config loader automatically injects `app.root` into the configuration, set to the resolved project root directory. This value is available both as a template variable during config rendering and as a config key at runtime:

```yaml title="config/storage.yaml"
root: "{{ app.root }}/storage/app"
```

```dart
final projectRoot = configValue<String>('app.root');
```

The `app.root` key is only set when it is not already explicitly defined in your configuration. The view and storage providers use it to resolve relative paths.

### `onConfigReload` hook

Providers that mix in `ProvidesDefaultConfig` can override `onConfigReload` to react when configuration files change in watch mode. The engine calls this method on every provider that implements it after a reload:

```dart
class MyProvider extends ServiceProvider with ProvidesDefaultConfig {
  @override
  ConfigDefaults get defaultConfig => const ConfigDefaults(docs: [/* ... */]);

  @override
  Future<void> onConfigReload(Container container, Config config) async {
    // Re-read your config and update cached state
    final newSettings = config.get<Map<String, dynamic>>('my_service');
    final service = await container.make<MyService>();
    service.reconfigure(newSettings);
  }
}
```

Registered providers such as logging, storage, views, sessions, and security
implement this hook, which is why their configuration changes take effect
...
Import `package:routed/routed.dart` to access `LiquidViewEngine` and `LiquidRoot` in code samples.
...
Static files can be registered directly on a router or engine, or through the
storage-backed `routed.static` provider:
...
Use the direct `static`/`staticFile` helpers above to serve assets today. For
example:
...
Configuration reloads do not install or update static mounts in the current
bundle. Programmatic direct helpers can supply a custom `file_system` (and
optional `root`) when mounting assets. This is useful for in-memory fixtures
during tests or when embedding templates from a virtual file system.
...
Cloud storage is not created from `storage.*` configuration by the current
Routed provider. The lower-level `server_storage` package exposes
`CloudStorageDisk`; construct a `StorageManager` with that disk and pass it to
`RoutedStorageProvider` when an application has a compatible cloud adapter.
This keeps cloud credentials and adapter setup in application code until a
first-party configuration driver is added.
...
Swap out Liquid entirely by implementing the `ViewEngine` interface and registering it with the `ViewEngineManager`:
...
class MustacheViewEngine implements ViewEngine {
  ...
  MustacheViewEngine({this.directory = 'views'});

  @override
  List<String> get extensions => ['.mustache', '.hbs'];

  @override
  Future<String> render(String name, [Map<String, dynamic>? data]) async {
    final template = await _loadTemplate(name);
    return _renderMustache(template, data ?? {});
  }

  @override
  Future<String> renderFile(String filePath, [Map<String, dynamic>? data]) async {
    final template = await File(filePath).readAsString();
    return _renderMustache(template, data ?? {});
  }

  // Your Mustache rendering implementation...
}

// Register it during provider boot or engine setup:
final viewManager = await engine.make<ViewEngineManager>();
viewManager.register(MustacheViewEngine(directory: 'templates'));
```

Pair this with the `view.*` and `storage.*` config so the active providers keep using the same disks and cache settings. Historical static mount configuration does not install routes in the current bundle.
...
| Static assets | `static.enabled`, `static.mounts[*]` via `routed.static` | `static(...)` / `staticFile(...)` direct helpers |
...
Routed includes an `Environment` wrapper that merges system variables with values loaded from `.env` files. Use the exported singleton `env` when you want safe reads or in-memory overrides:
