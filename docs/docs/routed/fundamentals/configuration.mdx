...
```dart
// One-liner — all builtins are included by Engine.create() automatically
final engine = await Engine.create(
  configItems: {
    'app.name': 'My App',
    'app.env': 'production',
...
`Engine.create` constructs an engine with all built-in providers by default,
boots service providers, and returns the initialized instance. Pass a custom
...
```dotenv title=".env"
SESSION__CONFIG__APP_KEY=base64:Z0x2...   # generate with `dart run routed key:generate`
```
...
The `app.root` key is only set when it is not already explicitly defined in your configuration. Providers like `ViewServiceProvider` and `StorageServiceProvider` use it to resolve relative paths.
...
All built-in providers (logging, storage, views, sessions, CORS, security, etc.) implement this hook, which is why configuration changes take effect without restarting the server.
...
> Store secrets in the manifest via Liquid placeholders (for example `session.config.app_key: "{{ session.config.app_key }}"` and `app.key: "{{ session.config.app_key }}"`). Keep the actual key in `.env` as `SESSION__CONFIG__APP_KEY` and generate it with `dart run routed key:generate`.
...
`dart run routed provider:list --config` is a quick way to verify the manifest-driven configuration after edits.
