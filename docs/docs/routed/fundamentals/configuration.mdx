...
- `http.features.<suffix>.enabled` controls opt-in behaviour. Providers (for example `routed.cors` or `routed.security`) read these flags so you can toggle middleware without editing code.
...
Each mount maps a request prefix (`route`) to a location managed by the storage provider. Provide `disk` to reuse a configured storage disk and set `path` relative to that disk (the legacy `directory` key is still honoured). When `index` is set, requests that land on the mount root (or a directory ending with a trailing slash) will serve that file. Enable `list_directories` to render directory listings when no index is available. Legacy values under `http.features.static` (e.g. `root`, `index`, `prefix`) are still honoured and translated into the same structure.
...
Providing a `storage` section is optional; if omitted, a default `local` disk rooted at `storage/app` is created.
...
| --- | --- | --- |
| Security feature flag | `http.features.security.enabled` | `EngineConfig(features: EngineFeatures(enableSecurityFeatures: true))` |
| Proxy support | `http.features.proxy.enabled`, `security.trusted_proxies.*` | `EngineConfig(features: EngineFeatures(enableProxySupport: true))`, `withTrustedProxies([...])` |
| Trusted platform headers | `http.features.trusted_platform.enabled`, `security.trusted_proxies.platform_header` | `EngineConfig(features: EngineFeatures(enableTrustedPlatform: true))` |
...
| Rate limiting | `rate_limit.enabled`, `rate_limit.backend`, `rate_limit.store`, `rate_limit.policies[*].*` | `RateLimitServiceProvider` (middleware auto-registered when `http.features.rate_limit.enabled`); configure policies via config. |
