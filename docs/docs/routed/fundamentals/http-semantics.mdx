...
Configure the default ETag behaviour through `RoutingServiceProvider`:
...
final engine = await Engine.create(
  providers: [
    ...
    RoutingServiceProvider(
      const RoutingConfig(etagStrategy: EtagStrategy.strong),
    ),
  ],
);
```
...
The default is enabled. Set `defaultOptionsEnabled: false` on
`RoutingConfig` when the application owns every `OPTIONS` handler.
...
All HTTP semantics settings are fields on the typed `RoutingConfig` passed to
`RoutingServiceProvider`.
