...
// Option 4: Use configItems for inline configuration (no YAML needed)
final engine = await Engine.create(
  configItems: {
    'app.name': 'My App',
    'mail.default': 'smtp',
...
The engine tracks provider types and skips duplicates by `runtimeType`. The first
registered instance of each type wins; subsequent instances are silently skipped.
Call `registerRoutedProviders()` once before using `Engine.builtins` or a
full-featured `Engine.create()` from `package:routed`.

This means:

- When you pass `configItems` to `Engine()` or `Engine.create()`, a
  `CoreServiceProvider` is automatically prepended. If `Engine.builtins` also
  includes one, the duplicate is skipped.
- Manifest-driven providers (from `config/http.yaml`) also respect deduplication—
  they will not override providers already registered in code.

### `Engine.defaultProviders` vs `Engine.builtins`

| Getter | Providers | Use case |
|--------|-----------|----------|
| `Engine.defaultProviders` | `CoreServiceProvider` + `RoutingServiceProvider` (2 total) | Minimal engine with config and routing only |
| `Engine.builtins` | The 14 providers registered by `package:routed`: core, routing, uploads, auth, logging, views, localization, observability, cache, sessions, storage, `rate_limit`, compression, and security | Full-featured engine after `registerRoutedProviders()` |

```dart
// Minimal engine — just config + routing
final engine = await Engine.create(providers: Engine.defaultProviders);

// Full-featured engine — all builtins (this is the default)
registerRoutedProviders();
final engine = await Engine.create();
```
...
## `CoreServiceProvider` modes

`CoreServiceProvider` supports three construction modes that control how configuration is loaded:

| Mode | Constructor | File I/O | Description |
|------|------------|----------|-------------|
| **Default (in-memory)** | `CoreServiceProvider({config, configItems})` | None | In-memory `Config` with framework defaults + optional overrides |
| **Loader** | `CoreServiceProvider.withLoader([options, configItems, config])` | Yes | Loads YAML/JSON from disk, env vars, `.env` files. Supports `watch` for hot reload |
| **Cached config** | `CoreServiceProvider.withCachedConfig(snapshot, {config, configItems})` | None | Uses a pre-built `ConfigSnapshot` from `routed config:cache`. No file I/O |

```dart
// Mode 1: In-memory only (default) — good for tests and simple apps
final engine = Engine(providers: [
  CoreServiceProvider(configItems: {'app.name': 'Test'}),
  RoutingServiceProvider(),
]);

// Mode 2: File-based loading — for apps with config/ directory
final engine = Engine(providers: [
  CoreServiceProvider.withLoader(ConfigLoaderOptions(
    configDir: 'config',
    watch: true,  // enables hot-reload of config files
  )),
  RoutingServiceProvider(),
]);

// Mode 3: Cached — for production (no file I/O at startup)
final snapshot = ConfigSnapshot.fromFile('.dart_tool/routed/config_cache.json');
final engine = Engine(providers: [
  CoreServiceProvider.withCachedConfig(snapshot),
  RoutingServiceProvider(),
]);
```

When using `Engine.create()` or passing `configItems` to the `Engine()` constructor, the framework automatically creates a default-mode `CoreServiceProvider` for you — no need to instantiate one manually.
