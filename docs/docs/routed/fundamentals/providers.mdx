...
| `Engine.builtins` | All 16 registered providers from `ProviderRegistry` (core, routing, cache, sessions, uploads, cors, security, logging, auth, observability, compression, rate_limit, storage, static, views, localization) | Full-featured engine (default for `Engine.create()`) |
