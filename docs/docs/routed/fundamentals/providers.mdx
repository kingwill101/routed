...
- `cleanup` runs when the engine shuts down for root-scoped providers, or after a
  request finishes for request-scoped providers.
...
Providers now declare a lifecycle scope. The engine uses this to decide when a
provider's `cleanup` hook should run.

- `ContainerScope.root` (default): cleaned up when the engine shuts down.
- `ContainerScope.request`: cleaned up after each request container is disposed.
- `ContainerScope.child`: for advanced use; not cleaned up automatically by the engine.

Mark request-scoped providers explicitly:

```dart title="Request-scoped provider"
class RequestMetricsProvider extends ServiceProvider {
  @override
