...
- Built-in providers are registered only in **full mode** (`Engine.full()` or
  `includeDefaultProviders: true`). To seed registries or add container services
  _before_ those providers finish wiring defaults, pass an `Engine` option:

```dart
final engine = Engine.full(options: [
  (engine) {
