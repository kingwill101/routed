...
The `routed.security` provider exposes the same proxy knobs via configuration:
...
The provider validates the configured proxy values during boot and reapplies
them on configuration reload. The re-exported `routed_security` primitives
