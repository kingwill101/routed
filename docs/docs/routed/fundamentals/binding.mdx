...
- `bind(target)` – auto-detects content type (`application/json`, `application/x-www-form-urlencoded`, `multipart/form-data`, `text/plain`) and merges parsed values into `target`. Existing keys in `target` are preserved unless overwritten by parsed values.
...
router.post('/users', (ctx) async {
  ...
  // Or specify binding type (all helpers return Futures)
...
`mustBindWith`, `bind`, `bindJSON`, and the other helpers all return `Future<void>`—always `await` them so that asynchronous binders (JSON, multipart, etc.) finish before continuing.
