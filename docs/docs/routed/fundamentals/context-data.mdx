...
## Engine & typed configuration access
...
Import the configuration type from the provider that owns it. Keep the
`RuntimeContext` used to create the engine at the application boundary when
you need deployment environment values; do not use a global configuration
map.
