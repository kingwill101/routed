...
`routed.security` validates this section and inserts the corresponding middleware automatically. Use `dart run routed provider:list --config` to review the active headers and spot malformed values before starting the server.
