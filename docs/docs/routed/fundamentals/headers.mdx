...
Rather not hand-roll them? `routed.security` handles trusted proxies,
request-size limits, CORS when enabled, and IP filtering; it does not install
security-header middleware. Keep these headers in explicit application
middleware. Note:
X-XSS-Protection is deprecated in
modern browsers; rely on a strong Content Security Policy (CSP) instead:

The explicit middleware example above is the runnable path for security
headers. Keep these policies in a typed provider or application middleware so
they are reviewed alongside the routes they protect.
