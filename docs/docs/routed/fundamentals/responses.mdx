...
> The view provider validates its configuration (`view.engine`, `view.directory`, `view.disk`, and `view.cache` types) and throws a `ProviderConfigException` if the manifest supplies the wrong shape. Use `dart run routed provider:list --config` to inspect the active view settings and confirm the provider is enabled.
...
The helpers above are small wrappers around `Render` implementations in `packages/routed/lib/src/render/*`. If you need explicit control, call `ctx.render` with a renderer instance or build a custom `Render` subclass.
...
Prefer configuration-driven mounts for larger apps (or when you want storage disks to back assets):
