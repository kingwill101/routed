...
`package:routed/middlewares.dart` includes helpers such as `timeoutMiddleware`, `basicAuth`, and `requestTrackerMiddleware`.
...
The built-in providers (`routed.sessions`, `routed.security`, `routed.cors`, etc.) register their middleware IDs in the registry when enabled via the manifest. Run `dart run routed_cli provider:list --config` to inspect which providers are active and which middleware IDs they contribute.
