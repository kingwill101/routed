...
Register global middleware on the engine and group or route middleware on the
router. This keeps ordering visible at the composition site:
...
Global middleware runs before groups, groups run before route middleware, and
provider-owned middleware is installed when that provider boots.
...
The current bundle contributes provider-owned middleware such as logging,
sessions, CORS, and IP filtering when those providers are registered. Header
and CSRF protections remain explicit application middleware.
...
- Use named middleware IDs (via `MiddlewareRef.of('id')`) when composing reusable middleware in code; register the implementation before the engine boots.
