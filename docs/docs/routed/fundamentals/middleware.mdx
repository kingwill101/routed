...
Routes and groups can opt out of specific middleware with `withoutMiddleware`.
Pass middleware identifiers (recommended) or middleware instances:

```dart
router.get('/public', publicHandler)
  .withoutMiddleware(['routed.rate_limit.middleware']);

router.group(
  path: '/api',
  middlewares: [MiddlewareRef.of('routed.rate_limit.middleware')],
  builder: (api) {
...
This removes matching middleware from the route/group stack. Named middleware
can be excluded even when it is registered globally.
