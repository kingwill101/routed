...
http:
  middleware:
    global:
      ...
      - routed.sessions.start  # available when routed.sessions is enabled
    groups:
      api:
...
Global IDs run before every request, while group entries apply when you call `router.group(name: 'api', ...)`. The CLI (`dart run routed_cli provider:list --config`) prints every registered middleware ID and surfaces validation errors (for example, unknown references). Providers contribute defaults via `http.middleware_sources.*`, keeping the manifest declarative.
...
`MiddlewareRef` is available from the central `package:routed/routed.dart` barrel.

The current bundle contributes middleware IDs from providers such as
`routed.logging`, `routed.sessions`, and `routed.security.ip_filter`.
Configured CORS is installed by `routed.security` as provider-managed
middleware, while header and CSRF protections remain explicit application
middleware. Run `dart run
routed_cli provider:list --config` to inspect the providers and middleware IDs
