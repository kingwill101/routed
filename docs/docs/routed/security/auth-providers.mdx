...
## Migration from routed_auth

Provider implementations were moved from `routed_auth` to `server_auth`.

- Old import: `package:routed_auth/routed_auth.dart`
- New import: `package:server_auth/server_auth.dart`

The `routed_auth` package is removed as part of the package split.

## Built-in Providers (server_auth)

The `server_auth` package provides pre-built OAuth providers following the NextAuth.js pattern.
...
The Dropbox provider in `server_auth` demonstrates this pattern.
