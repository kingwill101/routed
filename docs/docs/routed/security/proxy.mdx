...
`routed.security` validates the list (each entry must be a non-empty string) and updates the `TrustedProxyResolver` whenever configuration reloads. Run `dart run routed_cli provider:list --config` to check the effective proxy settings and catch mistakes early.
