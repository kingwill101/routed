...
- **Providers & configuration** – the current bundle registers core, routing, uploads, auth, logging, views, localization, observability, cache, sessions, storage, rate-limit, compression, and security providers. Each provider is explicitly configured with a typed Dart object and validated before boot.
- **CLI tooling** – `routed` scaffolds typed provider setup, runs a development server, inspects routes, and generates persisted specs.
