...
- **Providers & manifest** – each subsystem (cache, sessions, uploads, static assets, security, logging, etc.) ships as a service provider with sensible defaults. The manifest (`config/http.yaml`) controls which providers are active, and the CLI (`dart run routed provider:list --config`) shows active providers, merged defaults, and any validation errors.
- **CLI tooling** – `routed` scaffolds projects, runs a hot-reload dev server, inspects routes, and generates persisted specs.
