...
- **Providers & configuration** – each subsystem (cache, sessions, uploads, static assets, security, logging, etc.) ships as a service provider with sensible defaults. Feature toggles live in `config/http.yaml`, and the CLI (`dart run routed provider:list --config`) shows provider defaults and validation errors.
...
1. Build an `Engine` using `Engine.create()` which loads all built-in providers (config, routing, sessions, cache, logging, etc.).
