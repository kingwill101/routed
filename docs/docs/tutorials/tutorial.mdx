...
> In a larger app, keep the timeout middleware and feature settings at the Dart composition boundary: pass middleware to `Engine`, and pass each provider its typed configuration object. Provider validation runs before boot and reports invalid values before the server starts.
