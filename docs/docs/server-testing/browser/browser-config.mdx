...
For responsive-layout checks, resize the browser viewport inside the test. This
uses the public browser API and does not depend on generated device catalogs.

```dart
browserGroup('mobile layout', define: (getBrowser) {
  test('mobile layout', () async {
    final browser = getBrowser();
    await browser.window.resize(390, 844);
    await browser.visit('/');
...
Use `browser.emulation.setUserAgent(...)` or
`browser.emulation.setViewportSize(...)` when a test needs more specific
emulation controls.
