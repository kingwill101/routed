...
The `public` disk has its root at `"."` (the project directory), so `client/dist/assets` resolves correctly. Without it, paths resolve relative to the default disk's root (`storage/app`), causing 404s. See [Static assets](#static-assets) for the exact mount and disk setup used in this guide.
...
- **`{{ inertiaBootstrap }}`** -- renders the Inertia 3 bootstrap markup: a `<script data-page="app" type="application/json">` element with the page JSON plus the `<div id="app">` mount node, optionally containing SSR markup.
...
1. **First visit** -- the browser requests `/todos`. The server renders the Liquid template with the full HTML shell. The page object (`{component: "Todos", props: {todos: [...]}, url: "/todos", version: "dev"}`) is embedded in a `<script data-page="app" type="application/json">` element, and React mounts into `<div id="app">`.
...
Add a `public` disk with `root: "."` to `config/storage.yaml` and reference it with `disk: public` in `config/static.yaml`. See [Static assets](#static-assets) for the exact configuration.
...
- Read the [Routed Inertia reference](/docs/routed/) for shared props, SSR, error bags, and testing.
- Learn more about the [Routed Inertia reference](/docs/routed/) for shared props, SSR, and testing.
- See [Static assets](#static-assets) for mount and disk options.
- See [Root template](#root-template) for the Liquid template used by this tutorial.
