...
| DOC-005 | P1 | CLI deployment | Addressed | `RoutedNodeDeployCommand` contributes Cloudflare, Netlify, and Vercel support through `ServiceProvider.registerCliCommands` (`packages/routed_node/lib/src/cli/deploy.dart`). | Added a deployment section covering adapter discovery, prerequisites, target differences, generated artifacts, `--dry-run`, credentials/secrets, `--keep-vars`, compatibility dates, and recovery. | Docs |
