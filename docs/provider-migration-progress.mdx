...
- Engine bootstrap now returns a fully initialized instance, allowing immediate event hook registration. Use `Engine.defaultProviders` or pass custom providers via the `providers` parameter.
...
- CORS settings (`enabled`, origins, methods, headers, credentials, cache) resolve into core `EngineConfig` through `cors.*` (with legacy fallbacks), `withCors` writes through config for reload-safe updates, and `routed.security` installs the public CORS middleware when enabled.
- Compression settings (`enabled`, minimum length, algorithms, MIME allow/deny lists) are now handled by the `routed_http`-owned `routed.compression` provider; `package:routed` registers and re-exports it, gzip is applied to eligible buffered responses, and configuration reloads replace the middleware safely.
...
- Declarative static mounts now live in `routed_storage`: `routed.static` validates `static.mounts`, resolves configured storage disks, registers `GET`/`HEAD` routes, and removes/rebuilds them on config reload. Direct router/engine static helpers remain supported for code-defined routes.
...
- Provider registry coverage includes `routed.static`; the package registry test protects the full batteries-included catalogue.
- Config loader pre-processes YAML/JSON/TOML via Liquify, exposing `.env` + process environment values (including double-underscore nested keys) to config templates with `{{ }}` placeholders. Every key is mirrored under `env.*` (`APP_KEY` → `{{ env.APP_KEY }}`), and double underscores add a dotted alias alongside the original.
...
- `routed` ships configuration tooling (`config:init`, `config:publish`, `config:cache`, `config:clear`) plus runnable examples (`config_demo`, `route_events`) and updated docs.
...
- Removed the pre-routing middleware stage; global middlewares handle any host/application preflight middleware directly.
...
- **Passing locally:** the full `routed`, `routed_security`, and `routed_core` suites, plus targeted sessions, engine bootstrap, provider validation, CORS, and compression suites.
